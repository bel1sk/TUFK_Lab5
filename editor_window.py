...
from PyQt6.QtWidgets import (QMainWindow, QPlainTextEdit, QSplitter, QApplication, 
                             ...
                             QHeaderView, QLabel, QTextEdit, QDialog, QTextBrowser, QComboBox)
...
from analyzer import SyntaxAnalyzer, LexicalAnalyzer
from regex_analyzer import RegexAnalyzer
from course_docs import DOCS_CONTENT
...
class CompilerWindow(QMainWindow):
    def __init__(self):
        ...
        self.i18n = {
            'ru': {
                ...
                'new_file': 'Новый файл', 'results': 'Результаты поиска', 'errors': 'Ошибки',
                'row': 'Строка', 'col': 'Столбец', 'error_text': 'Текст ошибки',
                'substr_col': 'Найденная подстрока', 'pos_col': 'Начальная позиция', 'len_col': 'Длина',
                ...
                'help_text': 'Текстовый редактор с поддержкой регулярных выражений.\n\nРеализованные функции:\n- Поиск XML-комментариев\n- Поиск комментариев Pascal\n- Поиск RGB цветовых кодов\n- Подсветка найденных подстрок',
                'about_text': 'Реализация алгоритма поиска подстрок с помощью регулярных выражений\nКурсовой проект / Лабораторная работа\nАвтор: Гетман Денис Андреевич\nГруппа: АВТ-314\nГод: 2026',
                'invalid_frag': 'Неверный фрагмент',
                'no_errors': 'Нет данных для поиска.',
                'total_errors': 'Найдено совпадений: {}'
            },
            'en': {
                ...
                'new_file': 'New file', 'results': 'Search Results', 'errors': 'Errors',
                'row': 'Row', 'col': 'Column', 'error_text': 'Error Message',
                'substr_col': 'Found Substring', 'pos_col': 'Start Position', 'len_col': 'Length',
                ...
                'help_text': 'Text editor with regex search support.\n\nImplemented features:\n- XML comments search\n- Pascal comments search\n- RGB color codes search\n- Highlighting of found substrings',
                'about_text': 'Substring search algorithm using regular expressions\nCourse Project / Lab Work\nAuthor: Getman Denis Andreevich\nGroup: AVT-314\nYear: 2026',
                'invalid_frag': 'Invalid fragment',
                'no_errors': 'No data to search.',
                'total_errors': 'Matches found: {}'
            }
        }
    ...
    def translate_type(self, type_str):
        ...
        return types_map.get(type_str, type_str)

    def translate_msg(self, msg):
        if self.current_lang == 'ru':
            return msg
        errors_map = {
            "Ожидалось имя параметра (идентификатор)": "Expected parameter name (identifier)",
            "Ожидался тип данных (int, float)": "Expected data type (int, float)",
            "Ожидалось 'return'": "Expected 'return'",
            "Ожидался операнд (идентификатор, число, открывающая скобка)": "Expected operand (identifier, number, open parenthesis)",
            "Пропущен арифметический оператор или ';'": "Missing arithmetic operator or ';'",
            "Нарушен баланс скобок: лишняя ')'": "Parenthesis balance broken: extra ')'",
            "Нарушен баланс скобок: пропущена ')'": "Parenthesis balance broken: missing ')'",
            "Ожидалось ';'": "Expected ';'",
            "Ожидался аргумент после ','": "Expected argument after ','",
            "Ожидалось ',' или ')'": "Expected ',' or ')'",
            "Ожидалось 'auto'": "Expected 'auto'",
            "Ожидалось '='": "Expected '='",
            "Ожидалось '['": "Expected '['",
            "Ожидалось ']'": "Expected ']'",
            "Ожидалось '('": "Expected '('",
            "Ожидалось '{'": "Expected '{'",
            "Ожидалось '}'": "Expected '}'",
            "Недопустимая константа с плавающей точкой": "Invalid floating point constant",
            "Слишком длинный идентификатор (макс 10 символов)": "Identifier too long (max 10 characters)"
        }
        if msg in errors_map:
            return errors_map[msg]
        if msg.startswith("Недопустимый символ"):
            return msg.replace("Недопустимый символ", "Invalid character")
        return msg
    ...
    def setup_ui(self):
        ...
        self.output_tabs.addTab(self.ast_view, "AST Дерево")
    ...
    def setup_menus(self):
        ...
        for doc_title, content in DOCS_CONTENT.items():
            act = QAction(doc_title, self)
            act.triggered.connect(lambda checked, t=doc_title, c=content: self.show_doc(t, c))
            self.text_menu.addAction(act)
    ...
    def setup_toolbar(self):
        ...
        self.regex_combo.addItems([
            "1. XML-комментарии (<!--...-->)",
            "2. Pascal-комментарии ({...} или (*...*))",
            "3. RGB цветовой код (rgb(0-255, 0-255, 0-255))",
            "4. Pascal-комментарии (Конечный автомат)",
            "5. Семантический анализатор (C++ Lambda)"
        ])
        self.regex_combo.currentIndexChanged.connect(self.on_regex_changed)
        toolbar.addWidget(self.regex_combo)
        
        toolbar.addSeparator()
        toolbar.addAction(self.run_act)

    def on_regex_changed(self, index):
        editor = self.current_editor()
        if not editor: return
        
        if editor.current_file is None:
            samples = [
                "<div>\n    <!-- Основной блок контента -->\n    <p>Какой-то текст</p>\n    <!- Сломанный комментарий, он не должен найтись ->\n    <!-- \n      Многострочный\n      комментарий тоже\n      должен быть найден\n    -->\n</div>",
                "var x, y: integer; { Объявление переменных }\nbegin\n    x := 5; (* Инициализация\n    первой переменной *)\n    y := 10;\n    // Это однострочный комментарий в стиле C, он не должен найтись\nend. {Конец}",
                "body {\n    background-color: rgb(255, 0, 128);\n    color: rgb(300, 50, -10);\n    border: 1px solid RGB( 10, 20 , 30 );\n    fill: rgb(150,150,150);\n    shadow: rgba(0, 0, 0, 0.5);\n}",
                "var x, y: integer; { Автоматный поиск }\nbegin\n    x := 5; (* Инициализация\n    через DFA *)\n    y := 10; (( {Вложенная скобка} )* не ломает поиск\nend. {Конец автоматом}",
                "auto calc = [](int a, float b) {\n    return a + b;\n};"
            ]
            if 0 <= index < len(samples):
                editor.setPlainText(samples[index])
    ...
    def update_ui_texts(self):
        ...
        self.results_table.setHorizontalHeaderLabels([self.get_text('substr_col'), self.get_text('pos_col'), self.get_text('len_col')])
        self.errors_table.setHorizontalHeaderLabels([self.get_text('invalid_frag'), self.get_text('loc_col'), self.get_text('error_text')])
    ...
    def new_file(self, is_startup=False):
        editor = CodeEditor()
        if is_startup:
            sample = (
                "<div>\n"
    ...
    def show_doc(self, title, content):
        dlg = DocViewerDialog(title, content, self)
        dlg.exec()
    ...
    def on_lexeme_clicked(self, row, col):
        item_loc = self.results_table.item(row, 1)
        if item_loc:
            parts = item_loc.text().replace('(', '').replace(')', '').split(', ')
            r = int(parts[0].split(': ')[1])
            c = int(parts[1].split(': ')[1])
            length = int(self.results_table.item(row, 2).text())
            self.highlight_in_editor(r, c, c + length - 1)
    ...
    def run_analysis(self):
        ...
        if not code.strip():
            self.errors_label.setText(self.get_text('no_errors'))
            self.errors_label.setStyleSheet("padding: 5px; color: red;")
            return
            
        choice = self.regex_combo.currentIndex()
        if choice < 4:
            self.output_tabs.setCurrentIndex(0)
            matches = self.regex_analyzer.analyze(code, choice)
            
            self.errors_label.setText(self.get_text('total_errors').format(len(matches)))
            self.errors_label.setStyleSheet("padding: 5px; color: green;" if matches else "padding: 5px; color: red;")
            
            for i, match in enumerate(matches):
                self.results_table.insertRow(i)
                self.results_table.setItem(i, 0, QTableWidgetItem(match['substring']))
                self.results_table.setItem(i, 1, QTableWidgetItem(f"(Row: {match['row']}, Col: {match['col']})"))
                self.results_table.setItem(i, 2, QTableWidgetItem(str(match['length'])))
        else:
            if hasattr(self, 'ast_view'):
                self.output_tabs.setCurrentIndex(2)
            lex_analyzer = LexicalAnalyzer()
            lexemes, lex_errors = lex_analyzer.analyze(code)
            
            syntax_analyzer = SyntaxAnalyzer(lexemes)
            ast_root, syn_errors = syntax_analyzer.parse()
            
            all_errors = lex_errors + syn_errors
            
            if hasattr(self, 'ast_view'):
                if ast_root:
                    self.ast_view.setPlainText(ast_root.print_ast())
                else:
                    self.ast_view.setPlainText("Не удалось построить AST (критическая ошибка).")
            ...
            for i, error in enumerate(all_errors):
                ...
                self.errors_table.setItem(i, 0, QTableWidgetItem(error['lexeme']))
                self.errors_table.setItem(i, 1, QTableWidgetItem(f"(Row: {error['row']}, Col: {error['col']})"))
                self.errors_table.setItem(i, 2, QTableWidgetItem(error['message']))
